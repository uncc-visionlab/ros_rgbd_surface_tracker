...
gen_template_check_self = Template("""    if(!PyObject_TypeCheck(self, &${modulePrefix}_${name}_Type))
        return failmsgp("Incorrect type of self (must be '${name}' or its derivative)");
    $cname* _self_ = ${amp}((${modulePrefix}_${name}_t*)self)->v${get};
""")

gen_template_check_self_algo = Template("""    if(!PyObject_TypeCheck(self, &${modulePrefix}_${name}_Type))
        return failmsgp("Incorrect type of self (must be '${name}' or its derivative)");
    $cname* _self_ = dynamic_cast<$cname*>(${amp}((${modulePrefix}_${name}_t*)self)->v.get());
""")

gen_template_call_constructor_prelude = Template("""self = PyObject_NEW(${modulePrefix}_${name}_t, &${modulePrefix}_${name}_Type);
        new (&(self->v)) Ptr<$cname>(); // init Ptr with placement new
        if(self) """)

gen_template_call_constructor = Template("""self->v.reset(new ${cname}${args})""")

gen_template_simple_call_constructor_prelude = Template("""self = PyObject_NEW(${modulePrefix}_${name}_t, &${modulePrefix}_${name}_Type);
        if(self) """)
...
static PyObject* ${modulePrefix}_${name}_get_${member}(${modulePrefix}_${name}_t* p, void *closure)
{
    return ${modulePrefix}_from(dynamic_cast<$cname*>(p->v.get())${access}${member});
}
""")
...
static int ${modulePrefix}_${name}_set_${member}(${modulePrefix}_${name}_t* p, PyObject *value, void *closure)
{
    ...
    return ${modulePrefix}_to(value, dynamic_cast<$cname*>(p->v.get())${access}${member}) ? 0 : -1;
}
""")
...
class FuncInfo(object):
    ...
    def gen_code(self, all_classes):
        ...
        for v in self.variants:
            ...
            if self.isconstructor:
                code_decl += "    "+modulePrefix+"_%s_t* self = 0;\n" % selfinfo.name
...
class PythonWrapperGenerator(object):
    ...
    def gen(self, srcfiles, output_path):
        ...
        for hdr in srcfiles:
            ...
            self.code_include.write( '#include "{0}"\n'.format(hdr[hdr.rindex('ros_rgbd_surface_tracker/'):]) )
            # ARW 12/23/2020 END
            for decl in decls:
                name = decl[0]
...
if __name__ == "__main__":
    ...
    # ARW 12/23/2020 START
    dstdir = "/tmp/"
    modulePrefix = "pycv_uncc"
    if len(sys.argv) > 1:
        dstdir = sys.argv[1]
    if len(sys.argv) > 2:
        srcfiles = [f.strip() for f in open(sys.argv[2], 'r').readlines()]
        hdr_parser.opencv_hdr_list = srcfiles
        #srcfiles = ["src/bvmodule.hpp"]
    #dstdir = "/Users/vp/tmp"
    #if len(sys.argv) > 1:
    #    dstdir = sys.argv[1]
    #if len(sys.argv) > 2:
