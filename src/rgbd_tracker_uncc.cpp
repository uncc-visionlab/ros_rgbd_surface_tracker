...
//            estimateOdometryReprojectionError(rgbd_img_ptr);
        ...
        bool RgbdSurfaceTracker::estimateDeltaPoseReprojectionError(
            ...
            cv::Vec6f depth_gradient_vec, intensity_gradient_vec;
            cv::Mat error_hessian(6, 6, CV_32F);
            float* error_hessian_ptr;
            ...
            while (iterate) {
                ...
                for (int y = 0; y < height; ++y) {
                    for (int x = 0; x < width; ++x) {
                        ...
                        cv::Vec3f& pt = reinterpret_cast<cv::Vec3f*>(ptcloud1.data)[index];

                        if (!std::isnan(pt[2])) {

                            transformed_pt = rotation*pt + translation;
                            warped_px = rgbd_img2.project(static_cast<cv::Point3f>(transformed_pt));
                            ...
                            if (rgbd_img2.inImage(x0, y0) && rgbd_img2.inImage(x1, y1)) {
                                ...
                                if (!std::isnan(depth_img2_at_warped_px) && !std::isnan(depth2_gradx_at_warped_px) && !std::isnan(depth2_grady_at_warped_px)) {
                                    ...
                                    // compute residuals
                                    float depth_residual = depth_img2_at_warped_px - transformed_pt[2];
                                    const float& intensity_img1_at_xy = ((float *)intensity_img1.data)[index];
                                    float intensity_residual = intensity_img2_at_warped_px - intensity_img1_at_xy;
        ...
        //                                    );
                                    ...
                                    error_hessian_ptr = (float*)error_hessian.data;
